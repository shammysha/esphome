     ...
     while (ptr < (package->buff + package->size) && (metric = (type_digit_t*) ptr)) {
       switch (metric->type) {
         ...
         case TYPE_SIGNED_32:
           ...
           ESP_LOGV(TAG, "Обрабатываемый набор: %s", date);
           delay(10);
           ESP_LOGV(TAG, "Результат: %d, %f", reverse32((uint32_t) metric->value), (float) reverse32((uint32_t) metric->value) / 1000);
           delay(10);
         ...
         case TYPE_SIGNED_LONG:
           ...
           ESP_LOGV(TAG, "Обрабатываемый набор: %s", date);
           delay(10);
           ESP_LOGV(TAG, "Результат: %d, %f", reverse16((uint16_t) metric->value), (float) reverse16((uint16_t) metric->value) / 1000);
           delay(10);
...
void Nartis100::update() {
  if (this->phase_ != 0) {
    ...
    this->phase_ = 1;
    if (meter.server_upper_addr != this->logical_address_) {
       meter.server_upper_addr = this->logical_address_;
       ESP_LOGI(TAG, "Logical Address now is: %d", meter.server_upper_addr);
    }
    if (meter.server_lower_addr != this->physical_address_) {       
       meter.server_lower_addr = this->physical_address_;       
       ESP_LOGI(TAG, "Physical Address now is: %d", meter.server_lower_addr);         
    }
  }
}
